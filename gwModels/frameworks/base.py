...
class BaseEccentricHM:
    """
    Base class for converting multi-modal quasi-circular waveforms
    into multi-modal eccentric waveforms, given a quadrupolar eccentric
    waveform.
    ...
    Subclasses:
        - NRHME: for non-spinning systems
        - NRXHME: for non-precessing (spin-aligned) systems
    """

    # Empirical scaling constant relating amplitude and frequency modulations;
    # Eq(6) of https://arxiv.org/pdf/2403.15506
    AMP_TO_FREQ_MOD_FACTOR = 0.9

    def __init__(self, t_ecc, h_ecc_dict, t_cir, h_cir_dict,
        ...
        Parameters:
            ...
            h_cir_dict (dict): Dictionary of circular waveform modes.
                               Keys should be like 'h_l2m2', 'h_l2m1', etc. or (2,2), (2,1), etc.
        ...
        self.modekeytype = self._detect_mode_key_type()
    ...
    def _detect_mode_key_type(self):
        """Detect whether mode dictionary keys use string or tuple format."""
        VALID_STRING_KEYS = {
            'h_l2m0', 'h_l2m1', 'h_l2m2', 'h_l3m0', 'h_l3m1', 'h_l3m2', 'h_l3m3',
        ...
        first_key = self.modelist[0]
        if first_key in VALID_STRING_KEYS:
            return 'h_llmm'
        elif first_key in VALID_TUPLE_KEYS:
            return '(l,m)'
        else:
            raise ValueError(
                "Mode key not recognized. Please use dictionary keys as '(2,2)' or 'h_l2m2' format"
            )

    def _get_quadrupole_key(self):
        """Return the dictionary key for the (2,2) mode based on the key convention."""
        if self.modekeytype == 'h_llmm':
            return 'h_l2m2'
        return (2, 2)

    def _get_ell(self, mode):
        """Extract the ell value from a mode key."""
        if self.modekeytype == '(l,m)':
            return float(mode[0])
        return float(mode.rsplit("_l")[-1].rsplit("m")[0])
    ...
    def align_peaks(self):
        ...
        """
        quad_key = self._get_quadrupole_key()
        tpeak_cir = get_peak(self.t_cir, abs(self.h_cir_dict[quad_key]))[0]
        tpeak_ecc = get_peak(self.t_ecc, abs(self.h_ecc_dict[quad_key]))[0]
    ...
    def obtain_amplitude_modulation(self):
        ...
        Eq(4) of https://arxiv.org/pdf/2403.15506
        """
        quad_key = self._get_quadrupole_key()
        ecc_quadrupole_amp = abs(self.ecc_wfobj.h_transform[quad_key])
        cir_quadrupole_amp = abs(self.cir_wfobj.h_transform[quad_key])
    ...
    def obtain_orbfreq_modulation(self):
        ...
        Eq(6) and Eq(10) of https://arxiv.org/pdf/2403.15506
        """
        quad_key = self._get_quadrupole_key()
        if not self.get_orbfreq_mod_from_amp_mod:
            ecc_quadrupole_orbfreq = get_frequency(self.t_common, self.ecc_wfobj.h_transform[quad_key])
            cir_quadrupole_orbfreq = get_frequency(self.t_common, self.cir_wfobj.h_transform[quad_key])
            return (ecc_quadrupole_orbfreq - cir_quadrupole_orbfreq) / cir_quadrupole_orbfreq
        else:
            return self.xi_amp * self.AMP_TO_FREQ_MOD_FACTOR

    def twist_mode_amplitude(self, mode):
        """
        Convert the amplitude of a circular mode to its corresponding eccentric mode amplitude.
        Eq(9) of https://arxiv.org/pdf/2403.15506

        Parameters:
            mode: The mode identifier (e.g., 'h_l2m2' or (2,2)).

        Returns:
            array: The projected amplitude of the corresponding eccentric mode.
        """
        ell = self._get_ell(mode)
    ...
    def twist_mode_orbital_frequency(self, mode):
        ...
        Eq(10) of https://arxiv.org/pdf/2403.15506

        Parameters:
            mode: The mode identifier (e.g., 'h_l2m2' or (2,2)).

        Returns:
            array: The projected orbital frequency of the corresponding eccentric mode.
        """
    ...
    def twist_mode_phase(self, mode):
        ...
        Eq(11) of https://arxiv.org/pdf/2403.15506

        Parameters:
            mode: The mode identifier (e.g., 'h_l2m2' or (2,2)).

        Returns:
            array: The computed phase of the corresponding eccentric mode.
                   The integration constant is set to 0 (fixed later).
        """
        omega = self.twist_mode_orbital_frequency(mode)
        return scipy.integrate.cumulative_trapezoid(omega, self.t_common, initial=0)
    ...
    def twist_modes(self):
        ...
        Returns:
            dict: A dictionary containing the transformed eccentric waveform modes.
                  The amplitude and phase of the (2,2) mode remain unchanged,
        ...
        for mode in self.modelist:
            ...
            # Phase alignment: initial phase equals the initial phase of the circular waveform
            # Eq(11) of https://arxiv.org/pdf/2403.15506
